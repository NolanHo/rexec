...
- **Unimplemented routing directives are reported, not silently dropped.** `ProxyJump`/`ProxyCommand` are not implemented: rexec connects to the target directly and prints one warning line naming the directive and the address it actually used (`⚠ ssh config: proxyjump=… for <host> is not implemented; connecting directly to …`) instead of letting the run die in a bare connect timeout. Every other unsupported directive (`SendEnv`, `StrictHostKeyChecking`, `LocalForward`, …) stays silent.
